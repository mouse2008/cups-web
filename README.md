...
- **后台配置**：管理员可在后台配置 LDAP 连接参数并启用 LDAP 登录
- **首次登录自动建档**：LDAP 用户第一次成功登录时，会自动在本地 `users` 表创建对应记录
- **手动与定时同步**：支持管理员手动触发同步，也支持按配置周期执行定时同步
- **角色本地管理**：LDAP 用户的 `admin` / `user` 角色仍由本地后台管理，不从 LDAP 覆盖
- **应用内不能改 LDAP 密码**：LDAP 用户不能在本应用里修改密码
- **首版删除限制**：当前首版不支持在应用内删除 LDAP 用户
